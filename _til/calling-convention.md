...
`call`을 하게 되면 반환 주소(`add esp, 32의 주소`)를 스택에 push하고 callee의 주소로 이동한다.

이후 callee가 끝난 뒤 `add esp, 32`로 실행 흐름이 돌아오고, `esp` 주소를 옮기며 callee를 호출하기 전에 매개변수를 전달하기 위해 할당한 공간을 정리하고 callee가 `eax`에 저장한 반환값을 스택에 저장한다.
